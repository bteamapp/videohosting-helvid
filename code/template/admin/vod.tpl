                ...
                <table class="layui-table">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th>ID</th>
                            <th>名称</th>
                            <th class="hide">服务器</th>
                            <th class="hide">系统分类</th>
                            <th class="hide">私人分类</th>
                            <th class="hide">播放次数</th>
                            <th>会员ID</th>
                            <th class="hide">时长</th>
                            <th class="hide">大小</th>
                            <th>状态</th>
                    ...
                    <tbody>
                    <?php
                        if(empty($vod)) echo '<tr><td align="center" height="50" colspan="13">没有找到相关记录</td></tr>';
                        foreach ($vod as $row) {
                            if($row->zt==1){
			...
			function get_zt(){
			    $.post('<?=links('vod','ajax')?>',{did:v.join(',')},function(d){
					d.forEach(function(value,i){
						if(d[i].zt == 1){
							$("#zm_"+value.id).html('<font color=#1e9fff>转码中..</font>');
						} else if(value.zt == 2){
							$("#zm_"+value.id).html('<font color=green>转码完成</font>');
						} else if(value.zt == 3){
							$("#zm_"+value.id).html('<font color=red>转码失败</font>');
						}
					});
				},'json');
			}
