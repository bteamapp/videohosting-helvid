...
<style>
	...
	h5 span{float: right;color: red;width: 20px;text-align: center;height: 20px;font-size: 16px;cursor: pointer;}
</style>
                        ...
                        <div class="form-horizontal">
                            <div class="form-body" style="min-height: 700px;">
                 				...
                 				<div class="form-group">
                                	...
                                	<div class="col-md-6">
                              			<select id="fid" name="fid" class="bs-select form-control" title="请选择服务器...">
	                                        <?php
											if($fid > 0) echo '<option value="0">请选择服务器...</option>';
...
$(document).ready(function() {
	...
	$('#picker').click(function() {
		...
		if(fid == 0){
			layer.msg('请先选择上传服务器',{icon:2});
			return false;
		}
	});
});
var zmurl = (("https:" == document.location.protocol) ? " https://" : " http://")+'<?=Web_Zm_Url.Web_Path?>index.php/ting';
var upmsg = 0;
...
uploader.on( 'fileQueued', function( file ) {
	$('#thelist').append( '<div id="' + file.id + '" class="item">' +
		'<h5 class="info">' + file.name + '<span title="删除" onclick="del(\''+file.id+'\')">x</span></h5>' +
...
$('#ctlBtn').click(function(event) {
	...
	uploader.on('uploadProgress', function( file, percentage ) {
		...
		$li.find('p.state').html('<font color=blue>上传中...(速度:'+sd+'/s)</font>');
