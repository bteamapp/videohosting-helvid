...
if(isset($_GET['ac'])){
	...
	if($_GET['ac'] == 'set' && isset($_POST)){
		...
		$res = write_file(FCPATH.'config.php',$strs);
		if($res){
			echo 'ok';
		}else{
			echo 'err edit file';
		...
		echo 'ok';
	}elseif($_GET['ac'] == 'init'){ //判断转码完成

		$m3u8_path = Zm_Dir.date('Ym',$_POST['addtime']).'/'.date('d',$_POST['addtime']).'/'.$_POST['vid'].'/playlist.m3u8';
        if(substr($m3u8_path,0,2) == './') $m3u8_path = FCPATH.substr($m3u8_path,2);
        if(file_exists($m3u8_path)){
			$m3u8 = file_get_contents($m3u8_path);
			if(strpos($m3u8,'#EXTINF:') !== false){
				$zt = 2;
			}else{
				$zt = 1;
			}
